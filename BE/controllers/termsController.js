...
const createTermRule = async (req, res) => {
  ...
  try {
    const existing = await prisma.ruleCategory.findFirst({
      where: { category: 'terms', slug: categorySlug }
    });
    ...
    // ใช้ Transaction เพื่อสร้างข้อมูลหัวข้อหลักและข้อมูลย่อยพร้อมผูก categoryId ให้ถูกต้อง
            ...
            data: {
              ...
              rules: {
                create: rulesList.map((item) => ({
                  categoryId: newCategory.id, // ระบุ categoryId ให้ครบถ้วนตาม Schema
                  ruleText: item.text,
...
const deleteTermRule = async (req, res) => {
  try {
    ...
    const cat = await prisma.ruleCategory.findFirst({
      where: { id: categoryId, category: 'terms' }
    });
