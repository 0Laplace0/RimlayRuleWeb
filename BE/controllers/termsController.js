...
const getTermsAndConditions = async (req, res) => {
  try {
    // 1. ดึงข้อมูลหมวดหมู่หลักทั้งหมดที่เป็น terms
    const [categories] = await pool.query('SELECT * FROM rule_categories WHERE category = "terms" ORDER BY id ASC');
...
const createTermRule = async (req, res) => {
  ...
  try {
    ...
    const [existing] = await connection.query(
      'SELECT id FROM rule_categories WHERE category = "terms" AND slug = ?', 
      [categorySlug]
    );
    
    if (existing.length > 0) {
      await connection.rollback();
      return res.status(400).json({ success: false, message: 'ชื่อหัวข้อ Terms & Conditions นี้มีอยู่แล้วในระบบ' });
    }

    // 1. บันทึกหัวข้อหลัก
    const [catResult] = await connection.query(
      'INSERT INTO rule_categories (category, name, slug) VALUES (?, ?, ?)',
      ['terms', title, categorySlug]
    );
...
const updateTermRule = async (req, res) => {
  const connection = await pool.getConnection();
  try {
    await connection.beginTransaction();
    ...
    const [existing] = await connection.query(
      'SELECT id FROM rule_categories WHERE category = "terms" AND slug = ? AND id != ?', 
      [categorySlug, id]
    );
    if (existing.length > 0) {
      await connection.rollback();
    ...
    // 1. อัปเดตหัวข้อหลัก
    await connection.query(
      'UPDATE rule_categories SET name = ?, slug = ? WHERE id = ? AND category = "terms"',
      [title, categorySlug, id]
    );

    // 2. เคลียร์ข้อมูลลูกเก่าทิ้ง (ON DELETE CASCADE จะทำงานถ้า DB ตั้งไว้ แต่สั่งลบเพื่อความชัวร์)
...
const deleteTermRule = async (req, res) => {
  try {
    const { id } = req.params;
    // ON DELETE CASCADE จะเคลียร์ข้อมูลย่อยใน DB ให้อัตโนมัติ
    const [result] = await pool.query('DELETE FROM rule_categories WHERE id = ? AND category = "terms"', [id]);
