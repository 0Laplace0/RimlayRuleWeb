...
const dataFilePath = path.join(dataDir, 'councilRules.json');
...
exports.getCouncilRules = async (req, res) => {
  try {
    const data = getStoredData();
    res.status(200).json({ success: true, data });
  } catch (error) {
    console.error('Error in getCouncilRules:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// 2. Create Category/Section
exports.createCouncilRuleCategory = async (req, res) => {
  try {
    ...
    const images = files.map((file, index) => {
      ...
      return {
        ...
        imageUrl: `/uploads/council-rules/${file.filename}`,
    ...
    console.error('Error in createCouncilRuleCategory:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// 3. Update Category/Section
exports.updateCouncilRuleCategory = async (req, res) => {
  try {
    const { id } = req.params;
    let { title, type, items, subGroups, footerNote } = req.body;
    const files = req.files || [];
    ...
    console.error('Error in updateCouncilRuleCategory:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};

// 4. Delete Category/Section
exports.deleteCouncilRuleCategory = async (req, res) => {
  try {
    const { id } = req.params;
    let data = getStoredData();
    ...
    console.error('Error in deleteCouncilRuleCategory:', error);
    res.status(500).json({ success: false, message: error.message });
  }
};