...
const RoleplayRulesCRUD = () => {
  ...
  const fetchRules = async () => {
    try {
      ...
      swalUtils.error('เกิดข้อผิดพลาด!', 'ไม่สามารถดึงข้อมูลกฎระเบียบ Roleplay ได้');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    ...
    const actionTitle = view === 'add' ? 'ตรวจสอบการเพิ่มข้อมูลกฎ Roleplay' : 'ตรวจสอบการแก้ไขข้อมูลกฎ Roleplay';
    ...
    try {
      ...
      if (view === 'add') {
        ...
        swalUtils.success('เพิ่มข้อมูลกฎ Roleplay สำเร็จแล้ว!');
      } else {
        await axios.put(`${API_URL}/update/${form.id}`, payload, config);
        swalUtils.success('อัปเดตข้อมูลกฎ Roleplay สำเร็จแล้ว!');
      }
      ...
      {view === 'table' ? (
        <div>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <h2 className="text-2xl font-bold text-white">จัดการข้อมูล: กฎระเบียบ Roleplay</h2>
        ...
        <div className="space-y-6">
          <div className="bg-[#1e293b] border border-indigo-950/60 py-4 px-6 rounded-lg text-center shadow-lg">
            <h1 className="text-lg font-bold text-indigo-300 tracking-wide">
              :: {view === 'add' ? 'เพิ่มข้อมูลกฎ Roleplay' : 'แก้ไขข้อมูลกฎ Roleplay'} ::
            </h1>
          </div>
...
export default RoleplayRulesCRUD;