...
const RefundRulesCRUD = () => {
  ...
  }, [filteredRules, totalPages, currentPage]);
  ...
  const handleAddSubGroup = () => {
    setForm({
      ...form,
      subGroups: [...form.subGroups, { subId: `temp_${Date.now()}_${Math.random()}`, subTitle: '', rules: [] }]
    });
  };
        ...
        if (sg.subId === subId || sg.id === subId) {
          ...
          const updatedRules = rulesList.map(r => {
            if (r.ruleId === ruleId || r.id === ruleId) {
              ...
              const newSubItem = { subItemId: `temp_${Date.now()}_${Math.random()}`, text: '' };
  ...
  const handleSubmit = async (e) => {
    ...
    const actionTitle = view === 'add' ? 'ตรวจสอบการเพิ่มข้อมูล Refund Policy' : 'ตรวจสอบการแก้ไขข้อมูล Refund Policy';
    ...
    try {
      ...
      if (view === 'add') {
        ...
        swalUtils.success('เพิ่มข้อมูล Refund Policy สำเร็จแล้ว!');
      } else {
        await axios.put(`${API_URL}/update/${form.id}`, payload, config);
        swalUtils.success('อัปเดตข้อมูล Refund Policy สำเร็จแล้ว!');
      }
      ...
      {view === 'table' ? (
        <div>
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 mb-6">
            <h2 className="text-2xl font-bold text-white">จัดการข้อมูล: Refund Policy</h2>
                ...
                {loading ? (
                  ...
                  <tr key={item.id} className="hover:bg-indigo-950/10 transition">
                    ...
                    <td className="p-4 text-center">
                      <button type="button" onClick={() => handleOpenEdit(item)} className="px-4 py-1.5 bg-amber-500 hover:bg-amber-400 text-black text-xs font-bold rounded-full transition shadow-md cursor-pointer">edit</button>
                    </td>
                    <td className="p-4 text-center">
                      <button type="button" onClick={() => handleDelete(item)} className="px-4 py-1.5 bg-rose-600 hover:bg-rose-500 text-white text-xs font-bold rounded-full transition shadow-md cursor-pointer">delete</button>
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="space-y-6">
          <div className="bg-[#1e293b] border border-indigo-950/60 py-4 px-6 rounded-lg text-center shadow-lg">
            <h1 className="text-lg font-bold text-indigo-300 tracking-wide">
              :: {view === 'add' ? 'เพิ่มข้อมูล Refund Policy' : 'แก้ไขข้อมูล Refund Policy'} ::
            </h1>
          </div>
                  ...
                  <div key={subKey} className="bg-[#1e293b]/80 p-5 rounded-2xl border border-indigo-900/30 space-y-5 relative shadow-md">
                    ...
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <button 
                        ...
                        className="p-2.5 bg-rose-500/10 text-rose-500 hover:bg-rose-500 hover:text-white rounded-xl transition cursor-pointer shrink-0" 
                    ...
                    <div className="space-y-4 pt-2">
                      ...
                      <button type="button" onClick={() => handleAddRule(subKey)} className="mt-2 text-xs text-indigo-400 hover:text-indigo-300 flex items-center gap-1 font-semibold cursor-pointer">
